  ...
  async vnpayReturn(req, res) {
    try {
      ...
      if (!isValid) {
        ...
        return res.redirect(
          `http://localhost:5173/?payment=error&message=${encodeURIComponent('Chữ ký không hợp lệ')}`
        );
      }
      ...
      if (responseCode === '00') {
        ...
        return res.redirect(
          `http://localhost:5173/?payment=success&orderId=${orderId}&amount=${amount}&transactionNo=${transactionNo}`
        );
      } else {
        // Thanh toán thất bại
        ...
        return res.redirect(
          `http://localhost:5173/?payment=error&orderId=${orderId}&message=${encodeURIComponent(errorMessage)}`
        );
      }
